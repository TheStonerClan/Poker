...
export default async function PlayerHistoryBody({
  ...
  const [profile, adminUser] = await Promise.all([
    loadPlayerProfile({ supabase, playerId, isSandbox, range }),
    // Soft check (no redirect) — this page is public, but an admin
    // viewing it also gets the "refresh impressions" control. See
    // lib/auth's getAdminUser doc comment.
    getAdminUser(),
  ]);
  if (!profile) notFound();
  const isAdminViewer = adminUser != null;
        ...
        {impression || isAdminViewer ? (
          <section className="rounded-md border border-gold/30 bg-gold/[0.04] p-4">
            <div className="flex items-start justify-between gap-3">
              ...
              {isAdminViewer ? (
                <RefreshImpressionsButton isSandbox={isSandbox} />
              ) : null}
            </div>
            {impression ? (
              <>
              ...
              <p className="text-sm italic text-fg/45">
